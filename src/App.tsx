...
type View = 'login' | 'schools' | 'dashboard' | 'directory' | 'profile' | 'announcements' | 'admin'
...
export default function App() {
  ...
  if (view === 'login') return <LoginScreen onLogin={async (identifier, password, selectedRole) => { const user = await signIn(identifier, password); const profile = user ? await getUserProfile(user.uid) : null; if (profile && profile.role !== selectedRole) throw new Error(`This account is registered as ${profile.role}.`); const userRole = profile?.role ?? selectedRole; setRole(userRole); setFacultyId(profile?.facultyId); if (userRole === 'admin') { setView('admin') } else { setView('schools') } }} />

  if (view === 'admin' && role === 'admin') return <AdminDashboard onLogout={() => { setRole('student'); setFacultyId(undefined); setSchool(null); setView('login') }} />
